...
window.addEventListener('scroll', function() {
    ...
    if (window.scrollY > scrollDistance) {
        header.classList.add('sticky'); // เพิ่มคลาส sticky
        mainLogo.src = '../logo/Logo2.png';
    } else {
        header.classList.remove('sticky'); // ลบคลาส sticky
...
async function fetchAndRenderProjects(searchTerm = '') {
    ...
    try {
        ...
        statusMessage.innerText = `❌ เกิดข้อผิดพลาดในการดึงข้อมูล: ${error.message}`;
